...
const AuctionsView: React.FC<AuctionsViewProps> = (
  ...
  const brokerSellerQuery = useStreamQueries(BrokerCustomerSeller);
  const brokerSellers = useMemo(() => {
    return brokerSellerQuery.contracts.map((c) => c.payload.brokerCustomer);
  }, [brokerSellerQuery.contracts]);
