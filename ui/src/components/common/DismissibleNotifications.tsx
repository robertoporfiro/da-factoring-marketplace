...
export const useDismissibleNotifications = () => {
  ...
  const relationshipRequestNotifications = useStreamQueries(
    DismissibleNotificationTemplate,
    () => [],
    [],
    (e) => {
    ...
    .contracts.filter((notification) => notification.payload.receiver === party)
    .map((notification) => (
      <DismissibleNotification
        key={notification.contractId}
        notification={makeContractInfo(notification)}
        notificationDismiss={async () =>
          await dismissNotification(notification.contractId)
        }
      />
    ));

  const dismissNotification = async (
    cid: ContractId<DismissibleNotificationTemplate>
  ) => {
    const choice =
      DismissibleNotificationTemplate.DismissibleNotification_Dismiss;
...
const DismissibleNotification: React.FC<DismissibleNotificationProps> = ({
  ...
  switch (senderRole) {
    case MarketRole.InvestorRole:
      name = (
        <>
          Investor <b>@{lookup.investorMap.get(sender)?.name || sender}</b>
        </>
      );
      break;
    case MarketRole.IssuerRole:
      name = (
        <>
          Issuer <b>@{lookup.issuerMap.get(sender)?.name || sender}</b>
        </>
      );
      break;
    case MarketRole.BrokerRole:
      name = (
        <>
          Broker <b>@{lookup.brokerMap.get(sender)?.name || sender}</b>
        </>
      );
      break;
    case MarketRole.ExchangeRole:
      name = (
        <>
          Exchange <b>@{lookup.exchangeMap.get(sender)?.name || sender}</b>
        </>
      );
      break;
    case MarketRole.CustodianRole:
      name = (
        <>
          Custodian <b>@{lookup.custodianMap.get(sender)?.name || sender}</b>
        </>
      );
      break;
    default:
  ...
  */
  return (
    <NotificationComponent>
      <p>
        Notification from {name}: {notification.contractData.text}
      </p>
